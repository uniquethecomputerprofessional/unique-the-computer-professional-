...
import { searchCertificate } from '../data/certificatesData';
import { INSTITUTE_CONTACT } from '../data/instituteData';
import { 
  ShieldCheck, Lock, Search, CheckCircle2, XCircle, 
  Download, Printer, Share2, Award, Calendar, MapPin, 
  GraduationCap, FileText, Phone, HelpCircle, Sparkles 
} from 'lucide-react';
...
export const VerifyPage: React.FC = () => {
  ...
  const handleVerify = (e: React.FormEvent) => {
    ...
    const result = searchCertificate(studentIdInput);
    setRecord(result);
    setSearched(true);
  };
  ...
  const handleQuickFill = (id: string) => {
    ...
    const result = searchCertificate(id);
    setRecord(result);
    setSearched(true);
  };
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-10 shadow-2xl space-y-8 relative overflow-hidden">
          ...
          <form onSubmit={handleVerify} className="space-y-4">
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <button
                  type="submit"
                  className="absolute right-2 top-2 bottom-2 px-6 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-bold text-xs uppercase tracking-wider hover:shadow-lg hover:shadow-blue-600/30 transition-all"
                ...
                {record ? (
                  ...
                  <div className="p-8 rounded-2xl bg-slate-950 border border-rose-500/30 text-center space-y-4">
                    ...
                    <p className="text-xs text-slate-300 max-w-md mx-auto leading-relaxed">
                      No matching student record was found for "<span className="text-rose-400 font-mono font-bold">{studentIdInput}</span>". Please double-check the Student ID printed on the physical certificate or contact our office.
                    </p>
          ...
          <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 text-xs text-slate-400 space-y-2">
            ...
            <p className="leading-relaxed">
              Every graduate from Unique The Computer Professional receives a unique Student ID and Registration Number upon completion of coursework and practical laboratory examinations. All digital verification records are cryptographically verified against institute archives.
            </p>
          </div>
